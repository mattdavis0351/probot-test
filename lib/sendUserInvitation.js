  ...
  try {
    ...
    if (inviteResult) {
      if (inviteResult.status === 201) {
        console.log("sendUserInvitation: invite returned 201 and was sucessful")
        return 'invitation sucessfully sent'
      } else if (inviteResult.status === 204) {
        console.log("sendUserInvitation: user is already a collaborator and status is 204")
        return 'User is already a collaborator'
      }else{
        console.log(`sendUserInvitation returned a status code of: ${inviteResult.status}`)
      }
    }
  } catch (error) {
    if (error.status === 404) {
